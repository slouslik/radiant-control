...
uint_fast8_t SmartThings::translatePayload(uint8_t *pBuf, uint_fast8_t nBuf)
{
	...
	if ((payloadStart != 0) && (payloadEnd != 0) && (payloadEnd - payloadStart > 4))
		...
		if (isRxLine(_pBufRX))
		{
			debugPrintBuffer("->| ", _pBufRX, _nBufRX);
			{
				...
				if (messageBufLength > 0)
				{
					...
					// NEW CODE: Skip the first 4 bytes (ID, Type, Length)
					if (messageBufLength > 4) {
					  _calloutFunction(String((char *)_pBufRX + 3)); 
					} else {
					  _calloutFunction(String((char *)_pBufRX));
					}
				}
				else
				{
...
{
	// 1. Increment the rolling sequence number (0-255)
	_nextSeqNum++;

	// 1.5 Clear the Serial Buffer - Use correct abstraction for all serial types
	while (st_available() > 0) {
		st_read(); 
	}
	delay(10); // Short stabilization delay

	// 2. Start the Zigbee 'raw' command string
	// Format: [Frame Control] [Seq] [Command ID] [Attr ID Low] [Attr ID High] [Data Type]
	// 00 = Global Frame, 0A = Report Attribute, 0A 00 = ProductCode Attr, 42 = String
	st_print(F("raw 0x0 { 00 "));
	
	// Ensure 2-digit hex for sequence number
	if (_nextSeqNum < 16) st_print('0');
	st_print(_nextSeqNum, HEX);
	st_print(F(" 0A 0A 00 42 "));

	if (_isDebugEnabled)
	{
		Serial.print(F("<-| raw 0x0 { 00 "));
		if (_nextSeqNum < 16) Serial.print('0');
		Serial.print(_nextSeqNum, HEX);
		Serial.print(F(" 0A 0A 00 42 "));
	}

	// 3. Send the String Length byte with leading zero protection
	uint8_t len = message.length();
	if (len < 16) st_print('0'); 
	st_print(len, HEX);
	st_print(' ');

	if (_isDebugEnabled)
	{
		if (len < 16) Serial.print('0');
		Serial.print(len, HEX);
		Serial.print(' ');
	}

	// 4. Send the character data as Hex with leading zeros
	for (int i = 0; i < message.length(); i++)
	{
		uint8_t c = (uint8_t)message[i];
		if (c < 16) st_print('0');  // Ensure 2-digit hex
		st_print(c, HEX);
		st_print(' ');
	...
	// 5. Terminate the raw command and send via the Shield
	st_print(F("}\nsend 0x0 1 1\n"));
...
void SmartThings::send(String message)
{
  // Increment the sequence number (0-255) for every message sent
  _nextSeqNum++;

  // ZCL Global Report Attribute Header:
  // [00] Frame Control
  // [XX] Sequence Number (Now Dynamic)
  // [0A] Command ID (Report Attributes)
  // [0A 00] Attribute ID (ProductCode)
  // [42] Data Type (String)
  st_print(F("raw 0x0 { 00 "));
  st_print(_nextSeqNum, HEX);
  st_print(F(" 0A 0A 00 42 "));
  ...
  // Zigbee Strings require a length prefix byte
  uint8_t len = message.length();
  st_print(len, HEX);
  st_print(' ');
  ...
  // Send the payload characters as hex
  for (int i = 0; i < message.length(); i++)
    {
